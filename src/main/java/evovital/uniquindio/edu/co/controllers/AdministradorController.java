...
public class AdministradorController {
    ...
    public ResponseEntity<MensajeDTO<Long>> actualizarMedico(@Valid @RequestBody MedicoDTO medicoDTO) throws Exception {
    ...
    public ResponseEntity<MensajeDTO<MedicoDTO>> eliminarMedico(@PathVariable Long id) throws Exception {
        return ResponseEntity.ok().body(new MensajeDTO<>(false, administradorService.eliminarMedico(id)));
    }

    @GetMapping("/listar/medico")
    public ResponseEntity<MensajeDTO<List<MedicoDTOAdmin>>> listarMedicos() throws Exception {
        return ResponseEntity.ok().body( new MensajeDTO<>(false, administradorService.listarMedicos()));
    }

    @GetMapping("/obtener/medico/{id}")
    public ResponseEntity<MensajeDTO<InfoMedicoDTO>> obtenerMedico(@PathVariable Long id) throws Exception {
        return ResponseEntity.ok().body( new MensajeDTO<>(false, administradorService.obtenerMedico(id)));
    }

    @GetMapping("/listar/pqrs")
    public ResponseEntity<MensajeDTO<List<PQRSDTOAdmin>>> listarPqrs() throws Exception {
        return ResponseEntity.ok().body( new MensajeDTO<>(false, administradorService.listarPQRS()) );
    }

    @PostMapping("/responder/pqrs")
    public ResponseEntity<MensajeDTO<Long>> responderPqrs(@Valid @RequestBody MensajeDTOUsuario mensajeDTOUsuario) throws Exception {
        return ResponseEntity.ok().body( new MensajeDTO<>(false, administradorService.responderPQRS(mensajeDTOUsuario)));
    }

    @GetMapping("/obtener/pqrs/{id}")
    public ResponseEntity<MensajeDTO<InfoPQRSDTO>> obtenerPqrs(@PathVariable Long id) throws Exception {
        return ResponseEntity.ok().body( new MensajeDTO<>(false, administradorService.verDetallePQRS(id)) );
    }

    @GetMapping("/listar/consulta")
    public ResponseEntity<MensajeDTO<List<ConsultaDTOAdmin>>> listarConsultas() throws Exception {
        return ResponseEntity.ok().body( new MensajeDTO<>(false, administradorService.listarConsultas()) );
    }

    @PostMapping("/crear/horario/{idMedico}")
    public ResponseEntity<MensajeDTO<Long>> crearHorario(@PathVariable Long idMedico, @Valid @RequestBody HorarioDTO horarioDTO) throws Exception {
        return ResponseEntity.ok().body( new MensajeDTO<>(false, administradorService.crearHorario(idMedico, horarioDTO)) );
    }

    @PutMapping("/actualizar/horario/{idMedico}")
    public ResponseEntity<MensajeDTO<Long>> actualizarHorario(@PathVariable Long idMedico, @Valid @RequestBody HorarioDTO horarioDTO) throws Exception {
        return ResponseEntity.ok().body( new MensajeDTO<>(false, administradorService.crearHorario(idMedico, horarioDTO)) );
    }
